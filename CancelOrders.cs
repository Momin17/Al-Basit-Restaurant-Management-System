...
namespace GDL
{
    public partial class CancelOrders : Form
    {
        ...
        private void CancelOrders_Load(object sender, EventArgs e)
        {
            ...
            dataGridView2.DataSource = s.search("select Invoice_NO, Sum(Price) as [Total Value],Sum(Qty) as [Quantity],date, username  from Sales where Order_Status !='Cancel' group by Invoice_NO, date,username order by Invoice_NO desc");
        }
       ...
       (e.KeyChar != '.'))
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (data != "")
            {
                ...
                MessageBox.Show(c.updateData("update sales set Order_Status='Cancel' where Invoice_No =" + metroTextBox1.Text) + "And Store =" + Program.shopid);
                dataGridView2.DataSource = s.search("select Invoice_NO, Sum(Price) as [Total Value],Sum(Qty) as [Quantity],date, username  from Sales where Order_Status !='Cancel' And Store = " + Program.shopid + "group by Invoice_NO, date,username order by Invoice_NO desc");
